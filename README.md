...
- `GET /api/v1/admin/sync/matches/by-puuid/{puuid}` - получение последних матчей игрока по puuid. Параметр count определяет размер окна поиска новых матчей и одновременно задает порог для синхронизации, т.е. поиск новых матчей будет продолжаться, пока кол-во новых матчей < count. Если в пределах текущего окна count находится последний свежий синхронизированный матч игрока (этот match_id хранится в БД в данных аккаунта игрока) и кол-во новых матчей все еще < count, то запускается backfill заполнение для дозагрузки истории матчей игрока. ВАЖНО! backfill заполнение и углубленный поиск по свежим матчам игрока не выполняется, если профиль игрока не синхронизирован в БД
